...
class OnsetDetectionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        window_size = 2*self.sample_delta +1
        ...
        lower_bound = 0 if offset-self.sample_delta < 0 else offset-self.sample_delta
        stacked_spectrograms_random_subset = stacked_spectrograms[:,:, lower_bound:offset+self.sample_delta + 1]
        ...
        if list(stacked_spectrograms_random_subset.shape) != [3, 80, window_size]:
            if offset - self.sample_delta < 0:
                stacked_spectrograms_random_subset = F.pad(stacked_spectrograms_random_subset, (self.sample_delta - offset, 0), "constant", 0)
            elif offset + self.sample_delta >= stacked_spectrograms.shape[2]:
                stacked_spectrograms_random_subset = F.pad(stacked_spectrograms_random_subset, (0, window_size - stacked_spectrograms_random_subset.shape[2]), "constant", 0)
        assert list(stacked_spectrograms_random_subset.shape) == [3, 80, window_size]
