...
def evaluate(pred, target, target_sample_rate, use_dbn=False):
    ...
    ref_beats = mir_eval.beat.trim_beats(ref_beats, min_beat_time=2)
    est_beats = mir_eval.beat.trim_beats(est_beats, min_beat_time=2)
    beat_scores = mir_eval.beat.evaluate(ref_beats, est_beats)

    # evaluate downbeats - trim beats before 5 seconds.
    ref_downbeats = mir_eval.beat.trim_beats(ref_downbeats, min_beat_time=2)
    est_downbeats = mir_eval.beat.trim_beats(est_downbeats, min_beat_time=2)
