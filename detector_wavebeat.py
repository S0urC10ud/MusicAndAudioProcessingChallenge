...
def detect_everything(filename, options, wavebeat_model):
    """
    ...
    beats = detect_beats(filename, wavebeat_model,
            sample_rate, signal, fps, spect, magspect, melspect,
...
def detect_beats(filename, wavebeat_model, sample_rate, signal, fps, spect, magspect, melspect,
                 odf_rate, odf, onsets, tempo, options):
    """
    ...
    beats, downbeats = wavebeat_model.predict_beats(filename)

    resulting_beats = beats
    #if len(beats)/beats[-1] * 60 > 200:
    #    print("Using only downbeats")
    #    resulting_beats = downbeats

    return resulting_beats.tolist()
...
def main():
    ...
    wavebeat_model = dsTCNModel.load_from_checkpoint(ckpts[-1])
    wavebeat_model.eval()
    ...
    for filename in infiles:
        results[filename.stem] = detect_everything(filename, options, wavebeat_model)
